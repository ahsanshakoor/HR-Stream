var params = {
    selector: "#svgChart",
    // dataLoadUrl: "http://127.0.0.1:8000/get_graph/",
    dataLoadUrl: "http://workdaystream.com/get_graph/",
...
function drawOrganizationChart(params) {
    ...
    function update(source, param) {
        ...
        function tooltipContent(item) {
            ...
            strVar += "    <!--";
            strVar += "    <div class=\"tooltip-image-wrapper\"> <img width=\"300\" src=\"\"> <\/div>";
            strVar += "-->";
            ...
            strVar += "      <a class=\"name\" href='/employee/profile/" + item.user_id + "/' target=\"\"> " + item.firstname + " "+ item.lastname + "<\/a>";
            strVar += "      <p class=\"position\">" + item.positionName + " <\/p>";
            strVar += "";
            strVar += "     <button class='" + (item.unit.type == 'business' ? " disabled " : "") + " btn btn-primary ' onclick='params.funcs.departmentClick(" + JSON.stringify(item.unit) + ")'>" + item.unit.value + "</button>";
            strVar += "      <h4 class=\"tags-wrapper\">             <span class=\"title\">";
            strVar += "        ";
            ...
            parentLoop:
                for (var i = 0; i < secondLevelChildren.length; i++) {
                    ...
                    // for (var j = 0; j < thirdLevelChildren.length; j++) {
                        ...
                        if (secondLevelChild.unit.value.trim() == item.value.trim()) {
                            clear(params.selector);

                            hide(['.btn-primary']);
                            show(['.btn-primary.btn-back', '.primary.btn-fullscreen', '.department-information']);
                            // set('.dept-name', item.value);

                            set('.dept-emp-count', "Employees Quantity - " + getEmployeesCount(secondLevelChild));
                            set('.dept-description', secondLevelChild.unit.desc);

                            params.oldData = params.data;

                            params.data = deepClone(secondLevelChild);
                            found = true;
                            break parentLoop;
                        }
                    // }
    ...
    function reflectResults(results) {
        var htmlStringArray = results.map(function (result) {
            ...
            strVar += "            <\/div>";
            strVar += "            <div >";
            strVar += "              <a target='_blank' href='" + getProfileUrl(result.user_id) + "'><button class='btn btn-primary'>View Profile<\/button><\/a>";
            strVar += "              <button class='btn btn-primary' onclick='params.funcs.locate(" + result.uniqueIdentifier + ")'>Locate <\/button>";
            strVar += "            <\/div>";
            strVar += "          <\/a>";
            strVar += "        <\/div>";
    ...
    function back() {
        ...
        show(['.btn-primary']);
        hide(['.customTooltip-wrapper', '.btn-primary.btn-back', '.department-information'])
        clear(params.selector);
